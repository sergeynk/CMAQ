...
! Source file: /work/MOD3DEV/tskipper/methane/cmaq/20240708_methane_sim1/UTIL/chemmech/input/ch4/mech_ch4.def
! for Mechanism Name: CH4                             
...
!    IHETERO   = Reactions list pointer to heteorogenous reactions and tables
      ...
      CHARACTER( 32 ), PARAMETER :: MECHNAME = 'CH4'

      INTEGER, PARAMETER :: N_GAS_CHEM_SPC =   3
      INTEGER, PARAMETER :: NUMB_MECH_SPC  =   3
...
! If present, the CHEMISTRY_SPC names and species type should agree with the CGRID_SPCS module
      ...
      DATA GAS_CHEM_SPC(   2 ) / 'LCH4OH          ' /
      DATA GAS_CHEM_SPC(   3 ) / 'LCH4CL          ' /
...
! the below values of CGRID_INDEX, SPECIES_TYPE, SPECIES_MOLWT, and CONVERT_CONC
      ...
      & MEMBER("LCH4OH          ",    2, "GC",   16.00D0, F), &
      & MEMBER("LCH4CL          ",    3, "GC",   16.00D0, F) /)

      DATA CHEMISTRY_SPC(   1 ), SPECIES_MOLWT(   1 ) / 'ECH4            ',   16.00D0 /
      DATA CHEMISTRY_SPC(   2 ), SPECIES_MOLWT(   2 ) / 'LCH4OH          ',   16.00D0 /
      DATA CHEMISTRY_SPC(   3 ), SPECIES_MOLWT(   3 ) / 'LCH4CL          ',   16.00D0 /


      DATA CGRID_INDEX(   1 ), SPECIES_TYPE(   1 ), CONVERT_CONC(   1 ) /    1, 'GC', F /  ! ECH4
      DATA CGRID_INDEX(   2 ), SPECIES_TYPE(   2 ), CONVERT_CONC(   2 ) /    2, 'GC', F /  ! LCH4OH
      DATA CGRID_INDEX(   3 ), SPECIES_TYPE(   3 ), CONVERT_CONC(   3 ) /    3, 'GC', F /  ! LCH4CL

! The below integers define the locations of mechanism species in the solver
! concentration array.

      INTEGER :: INDEX_ECH4   =    1
      INTEGER :: INDEX_LCH4OH =    2
      INTEGER :: INDEX_LCH4CL =    3

      INTEGER, PARAMETER :: N_ACT_SP =   3

      INTEGER, PARAMETER :: NRXNS =   2
      ...
      INTEGER, PARAMETER ::        NTHERMAL_RXNS    =    2
...
! Photolysis reactions information not available ...
     ...
     &      0,    0/
      ...
      INTEGER, PARAMETER :: NTERMS_JACOB =        9

      INTEGER, PARAMETER :: NSTEPS_JACOB =        4
